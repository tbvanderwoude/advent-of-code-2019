...
impl Moon
{
    ...
    {
        return (self.p.0.abs()+self.p.1.abs()+self.p.2.abs())*(self.v.0.abs()+self.v.1.abs()+self.v.2.abs());
    }
    ...
    {
        return format!("pos=<x = {0}, y = {1}>, z = {2}>, vel=<x = {3}, y = {4} z = {5}>",
...
pub fn load_moons(filename: &String) -> Vec<Moon>
{
    ...
    for s in split {
        ...
        moons.push(Moon{p:(x,y,z), v:(0,0,0)});
    }
    return moons;
}